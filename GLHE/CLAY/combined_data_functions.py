...
from GLHE.CLAY import events, globals
from globals import SLC_MAPPING_REVERSE_UNITS, SLC_MAPPING_REVERSE_NAMES
...
def output_plot_of_all_data(dataset: pd.DataFrame) -> None:
    ...
    plt.tight_layout()
    plt.savefig(
        globals.config["DIRECTORIES"]["OUTPUT_DIRECTORY"] + "/" + globals.config["LAKE_NAME"] + "_products_plot.png")
    pub.sendMessage(events.topics["output_file_event"],
                    message=events.OutputFileEvent(globals.config["LAKE_NAME"] + "_products_plot.png",
                                                   globals.config["DIRECTORIES"]["OUTPUT_DIRECTORY"] + "/" +
                                                   globals.config["LAKE_NAME"] + "_products_plot.png",
                                                   ".png",
...
def output_all_compiled_data_to_csv(filename: str, dataset: pd.DataFrame) -> None:
    """Output data into csv
        ...
        """
    logger.info("Output CSV written here: " + globals.config["DIRECTORIES"]["OUTPUT_DIRECTORY"] + "/" + filename)
    pub.sendMessage(events.topics["output_file_event"],
                    message=events.OutputFileEvent(filename,
                                                   globals.config["DIRECTORIES"]["OUTPUT_DIRECTORY"] + "/" + filename,
                                                   ".csv",
                                                   "A csv file of all data, the main product"))

    dataset.to_csv(globals.config["DIRECTORIES"]["OUTPUT_DIRECTORY"] + "/" + filename)
...
def present_mv_series_as_geospatial_at_date_time(date: pd.Timestamp, *dss: MVSeries) -> None:
    ...
    for counter, mvs in enumerate(dss):
        ...
        filename = mvs.variable_name + "_" + mvs.product_name + "_" + globals.config["LAKE_NAME"] + "_" + date.strftime(
            '%Y%m%d') + ".tif"
    ...
    zip_file = globals.config["DIRECTORIES"]["OUTPUT_DIRECTORY"] + "/" + globals.config[
...
def write_and_output_README(read_me_information: dict) -> None:
    ...
    """
    READ_ME_Name = globals.config["DIRECTORIES"]["OUTPUT_DIRECTORY"] + "/" + globals.config["LAKE_NAME"] + "_README"
    doc = snakemd.new_doc()
    doc.add_heading(globals.config["LAKE_NAME"] + " README Information!")
