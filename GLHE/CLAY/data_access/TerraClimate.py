...
class TerraClimate(data_access_parent_class.DataAccess):
    ...
    def product_driver(self, polygon, debug=False, run_cleanly=False) -> list[MVSeries]:
        ...
        #     )
        # )
        helpers.clean_up_specific_temporary_files("TerraClimate")
        # list_of_MVSeries = [precip_ds, evap_ds]
        self.send_data_product_event(
            events.DataProductRunEvent("TerraClimate", self.README_default_information)
        )
        self.logger.info("TerraClimate Driver Finished")
        # return list_of_MVSeries

    def call_TerraClimate_and_process(self, polygon) -> xr.Dataset:
        """Calls the TerraClimate API and processes the data"""
        ...
        pathname = 'http://thredds.northwestknowledge.net:8080/thredds/dodsC/agg_terraclimate_aet_1958_CurrentYear_GLOBE.nc'
        ...
        lat_index_min = (np.abs(lat-min_lat)).argmin()
        lat_index_max = (np.abs(lat-max_lat)).argmin()
        lon_index_min = (np.abs(lon-min_lon)).argmin()
        lon_index_max = (np.abs(lon-max_lon)).argmin()
        [lat_index_min,lon_index_min] = check_latlon_bounds(lat, lon, lat_index_min, lon_index_min, min_lat, min_lon)   
        [lat_index_max,lon_index_max] = check_latlon_bounds(lat, lon, lat_index_max, lon_index_max, max_lat, max_lon)
        if lat_index_min > lat_index_max:
            lat_index_min, lat_index_max = lat_index_max, lat_index_min
        if lon_index_min > lon_index_max:
            lon_index_min, lon_index_max = lon_index_max, lon_index_min

            
        dataset = dataset.isel(
            lat=slice(lat_index_min, lat_index_max + 1),
            lon=slice(lon_index_min, lon_index_max + 1)
        )
        ...
        try:
            ...
            self.logger.error(
                "TerraClimate subset Failed: Polygon is too small for TerraClimate, trying again with larger polygon"
            )
        ...
        # shelpers.pickle_var(dataset, dataset.attrs["product_name"])
        return dataset
