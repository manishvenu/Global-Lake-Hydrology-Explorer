...
class GriddedDataDisplay:
    ...
    def __init__(self, config: dict):
        ...
        with zipfile.ZipFile(
            os.path.join(config["GRIDDED_DATA_FOLDER"]), "r"
        ) as zip_ref:
            for filename in zip_ref.namelist():
                split_list = filename.split("_")
                metadata = {
                    "var": split_list[0],
                    "product": split_list[1],
                    "date": split_list[-1].split(".")[0],
                }

                file_path = os.path.join(self.dest_dir)
                zip_ref.extract(filename, file_path)
                self.tif_file_paths[metadata["var"] + "." + metadata["product"]] = (
    ...
    def get_points(self, key: str) -> hv.Points:
        ...
        out_csv = os.path.join(
            self.dest_dir,
            self.tif_file_paths[key].split("\\")[-1].split(".")[0] + ".csv",
        )
