...
class BaseTestCase(unittest.TestCase):
    ...
    def setUp(self):
        # self.terra_dataset = xr.load_dataset('tests/DummyData/TerraClimate_ppt_1958.nc')
        dims = ("time", "lat", "lon")
...
class MyTestCase(BaseTestCase):
    ...
    def test_name(self):
        self.random_dataset = xarray_helpers.label_xarray_dataset_with_product_name(
            self.random_dataset, "TerraClimateDummyData"
        )
        self.assertEqual(
            self.random_dataset.attrs["product_name"], "TerraClimateDummyData"
        )
    ...
    def test_plotting(self):
        csv_dataset = pd.read_csv(".temp/test.csv")
