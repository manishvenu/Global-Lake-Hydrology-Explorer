...
class BaseTestCase(unittest.TestCase):
    ...
    def setUp(self):
        os.chdir(
            r"C:\Users\manis\OneDrive - Umich\Documents\Global Lake Hydrology Explorer\GLHE"
        )
        ...
        self.terra_dataset = xarray_helpers.label_xarray_dataset_with_product_name(
            self.terra_dataset, "TerraClimateDummyData"
        )
        dims = ("time", "lat", "lon")
...
class MyTestCase(BaseTestCase):
    ...
    def setUpClass(cls):
        ...
        fh = logging.FileHandler(
            os.path.join(GLHE.CLAY.globals.LOGGING_DIRECTORY, "GLHE_root_test.log")
        )
    ...
    def test_name(self):
        ...
        self.assertEqual(
            self.terra_dataset.attrs["product_name"], "TerraClimateDummyData"
        )

    def test_unit_conversion(self):
        self.terra_dataset = xarray_helpers.convert_xarray_dataset_units(
            self.terra_dataset, "in", "ppt"
        )
        self.assertIsNotNone(self.terra_dataset)

    def test_xarray_untis_conversion_stuff(self):
        dataset = helpers.unpickle_var("NWM")
        dataset = xarray_helpers.rename_xarray_units(
            dataset, "m3/s", "inflow", "outflow"
        )
        dataset = xarray_helpers.convert_xarray_dataset_units(
            dataset, "m3/month", "inflow", "outflow"
        )
    ...
    def test_NWM_class(self):
        ...
        NWM_object = NWM.NWM()
        NWM_object.product_driver(
            polygon, True, True
        )  # NWM Great Salt Lake ID: 4603518
        self.assertEqual(1, 1)
